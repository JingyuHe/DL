...
class Weight(Layer):
    ...
    def call(self, inputs):
        ...
        positive_weight_norm, _ = tf.linalg.normalize(positive_weight, ord=1, axis=1)
        negative_weight = tf.keras.layers.Softmax(axis=[0, 2])(y)
        negative_weight_norm, _ = tf.linalg.normalize(negative_weight, ord=1, axis=1)
        return positive_weight - negative_weight, positive_weight_norm - negative_weight_norm
...
class DeepFactorModel(Model):
    ...
    def call(self, inputs, training=True):
        ...
        weight_nonorm, weight = self.weight_layer(normalized_Z)

        factor_nonorm = tf.reshape(
            self.factor_layer(weight_nonorm, ret), [-1, self.hidden_sizes[-1]]
        )
        ...
        return [deep_factor, Z, r, deep_beta, factor, weight, factor_nonorm, weight_nonorm]
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape, tf.GradientTape() as factor_tape, tf.GradientTape() as beta_tape:
            ...
            _, _, beta_f, _, _, _, _, _ = self(x, training=True)
...
def get_model_prediction(model, data, batch=60):
    ...
    for i in range(len(splits) - 1):
        ...
        factor_subset, char_subset, r, beta, factor, weight, factor_nonorm, weight_nonorm = model(data_subset, training=False)
        ...
        in_weight.append(np.array(weight))
        in_factor_nonorm.append(np.array(factor_nonorm))
        in_weight_nonorm.append(np.array(weight_nonorm))
    in_factors = np.vstack(in_factors)
    in_chars = np.vstack(in_chars)
    in_r = np.vstack(in_r)
    in_beta = np.vstack(in_beta)
    in_factor = np.vstack(in_factor)
    in_weight = np.vstack(in_weight)
    in_factor_nonorm = np.vstack(in_factor_nonorm)
    in_weight_nonorm = np.vstack(in_weight_nonorm)

    return in_factors, in_chars, in_r, in_beta, in_factor, in_weight, in_factor_nonorm, in_weight_nonorm
...
def sequential_deep_factor(
    ...
    if cv_index == 0: # [360, test 120]
        train_idx = [i for i in range(n_train * 3)]
        test_idx = [(i + n_train * 3) for i in range(n_train)]
    elif cv_index == 1: # [240, test 120, 120]
        test_idx = [(i + n_train * 2) for i in range(n_train)]
        train_idx = [i for i in range(n_train * 4) if i not in test_idx]
    elif cv_index == 2: # [120, test 120, 240]
        test_idx = [(i + n_train) for i in range(n_train)]
        train_idx = [i for i in range(n_train * 4) if i not in test_idx]
    elif cv_index == 3: # [test 120, 360]
        test_idx = [i for i in range(n_train)]
        train_idx = [i for i in range(n_train * 4) if i not in test_idx]
    else:
        train_idx = [i for i in range(n_train * 4)]
        test_idx = [(i + n_train * 4) for i in range(n_test)]
    ...
    for idx in range(n_factor):
        ...
        # in case GPU memory full
        in_factors, in_chars, in_r, in_beta, in_factor, in_weight, in_factors_nonorm, in_weight_nonorm = get_model_prediction(
            model, train_data, batch=60
        )
        out_factors, out_chars, out_r, out_beta, out_factor, out_weight, out_factors_nonorm, out_weight_nonorm = get_model_prediction(
        ...
        train_weight.append(in_weight)
        test_weight.append(out_weight)
        train_factors_nonorm.append(in_factors_nonorm)
        test_factors_nonorm.append(out_factors_nonorm)
        train_weight_nonorm.append(in_weight_nonorm)
        test_weight_nonorm.append(out_weight_nonorm)
        ...
        each_beta = np.vstack([in_beta, out_beta])
        each_beta = pd.concat(
            {i: pd.DataFrame(each_beta[i]) for i in range(T)}, names=["month"]
        )
        # each_beta.columns = [f"DB_{i}" for i in range(idx+2)]
        each_beta.to_csv(
            f"{result_path}/beta{benchmark}_{n_layer}_{n_factor}_{l1_lam_log}_{l1_lam_beta_log}_{idx}_cv{cv_index}.csv", index=False, encoding='utf_8_sig'
        )

        each_f_g = np.vstack([in_factor, out_factor])
        each_f_g = pd.concat(
            {i: pd.DataFrame(each_f_g[i].T) for i in range(T)}, names=["month"]
        )
        each_f_g.to_csv(
            f"{result_path}/f_g{benchmark}_{n_layer}_{n_factor}_{l1_lam_log}_{l1_lam_beta_log}_{idx}_cv{cv_index}.csv", index=False, encoding='utf_8_sig'
        )

    train_factors = np.hstack(train_factors)
    test_factors = np.hstack(test_factors)
    train_factors_nonorm = np.hstack(train_factors_nonorm)
    test_factors_nonorm = np.hstack(test_factors_nonorm)
    ...
    train_weight = np.stack(train_weight, -1).reshape(
        -1, n_stock, n_factor
    )
    test_weight = np.stack(test_weight, -1).reshape(
        -1, n_stock, n_factor
    )
    train_weight_nonorm = np.stack(train_weight_nonorm, -1).reshape(
        -1, n_stock, n_factor
    )
    test_weight_nonorm = np.stack(test_weight_nonorm, -1).reshape(
        -1, n_stock, n_factor
    )

    deep_factors = pd.DataFrame(np.vstack([train_factors, test_factors]))
    deep_factors.columns = [f"DF_{i}" for i in range(n_factor)]
    deep_factors_nonorm = pd.DataFrame(np.vstack([train_factors_nonorm, test_factors_nonorm]))
    deep_factors_nonorm.columns = [f"DF_{i}" for i in range(n_factor)]
    ...
    deep_weight = np.vstack([train_weight, test_weight])
    deep_weight = pd.concat(
        {i: pd.DataFrame(deep_weight[i]) for i in range(T)}, names=["month"]
    )
    deep_weight_nonorm = np.vstack([train_weight_nonorm, test_weight_nonorm])
    deep_weight_nonorm = pd.concat(
        {i: pd.DataFrame(deep_weight_nonorm[i]) for i in range(T)}, names=["month"]
    )
