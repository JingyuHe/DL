...
for n_layer, g_dim, l1 in itertools.product(
    ...
    # Set up Random Seed for each loop
    ...
    # 4 folders
    for j in [4, 0, 1, 2, 3]:
        deep_factors, deep_chars, deep_beta, rhat, loss, mse, f_g, gradient_factor, gradient_beta, weight, factor_nonorm, weight_nonorm = sequential_deep_factor(
            result_path,
            input_data,
            n_layer=n_layer,
            n_factor=n_factor,
        ...
        deep_chars.to_csv(
            f"{result_path}/chars{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv{j}.csv", index=False, encoding='utf_8_sig'
        )
        deep_beta.to_csv(
        ...
        weight.to_csv(
            f"{result_path}/weight{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv{j}.csv", index=False, encoding='utf_8_sig'
        )
        factor_nonorm.to_csv(
            f"{result_path}/factor_nonorm{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv{j}.csv", index=False, encoding='utf_8_sig'
        )
        weight_nonorm.to_csv(
            f"{result_path}/weight_nonorm{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv{j}.csv", index=False, encoding='utf_8_sig'
        )


        dc_list = [f"DC_{i}" for i in range(n_factor)]
        deep_chars = deep_chars.reset_index()
        deep_chars.columns = ['month', 'rank'] + dc_list
        for char in dc_list:
            temp_df = deep_chars[['month', 'rank', char]]
            temp_df = temp_df.pivot(index='rank', columns='month', values=char).reset_index()
            temp_df.to_csv(
                f"{result_path}/chars{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv{j}_{char}.csv", index=False, encoding='utf_8_sig'
            )
...
for g_dim in g_dim_list:
    for n_layer in layer_list:
        for l1 in l1_v_list:
            ...
            cv2 = pd.read_parquet(f"{result_path}/loss{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv2.parquet")
            cv3 = pd.read_parquet(f"{result_path}/loss{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv3.parquet")
            ...
            mse2 = pd.read_parquet(f"{result_path}/mse{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv2.parquet")
            mse3 = pd.read_parquet(f"{result_path}/mse{g_dim}_{n_layer}_{n_factor}_{l1}_{l1+l1_beta}_cv3.parquet")
            cv = (cv0 + cv1 + cv2 + cv3) / 4.0
            cv = cv.iloc[99]
            mse_total = (mse0 + mse1 + mse2 + mse3) / 4.0
...
for g in g_dim_list:
    for layer in layer_list:
        ...
        loss_mat = np.array(temp_loss[['Loss_2','Loss_3','Loss_4']])
...
cv_loss['model_list'] = cv_loss.apply(lambda col: 'factors'+str(int(col['g_dim']))+'_'+str(int(col['n_layer']))+'_5_'+str(int(col['l1']))+'_'+str(int(col['l2']))+'_cv4_'+str(int(col['min_n_factor'])), axis=1)
...
for g in g_dim_list:
    for layer in layer_list:
        ...
        mse_mat = np.array(temp_mse[['MSE_2','MSE_3','MSE_4']])
...
cv_mse['model_list'] = cv_mse.apply(lambda col: 'factors'+str(int(col['g_dim']))+'_'+str(int(col['n_layer']))+'_5_'+str(int(col['l1']))+'_'+str(int(col['l2']))+'_cv4_'+str(int(col['min_n_factor'])), axis=1)
